...
class ASGShopCardDetails extends AsgShopCardMixin(AppElement) {
  ...
  async __addToCardFabClicked() {
    try {
      ...
      const card = this.$.controls.addSelectedToCard();
      if (this._isBuylist) {
        this.fire('asg-shop-card-details-buylist-add-to-cart', {card, buylist: true});
        return;
      }
  ...
  async open(detail) {
    const {card, image, x, y, isBuylist, buylistRules} = detail;
    this._isBuylist = isBuylist;
    this._buylistRules = buylistRules;
